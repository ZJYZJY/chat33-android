...
public class PullExtendLayout extends LinearLayout implements IPullToExtend {
    ...
    protected void onFinishInflate() {
        ...
        if (childCount == 2) {
            ...
            throw new IllegalStateException(getContext().getString(R.string.chat_error_layout_count));
        }
        if (mRefreshableView == null) {
            throw new IllegalStateException(getContext().getString(R.string.chat_error_layout_empty));
        }
    ...
    public final boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                // 下滑
                boolean down = deltaY > mTouchSlop;
                // 上滑
                boolean up = deltaY < -mTouchSlop / 3 &&
                        (mHeaderLayout.getState() == IExtendLayout.State.arrivedListHeight
                                || mHeaderLayout.getState() == IExtendLayout.State.beyondListHeight);
                // 上下滑动扩展菜单时拦截事件
                if (down || up) {
                    int position = ((ViewPager) mRefreshableView).getCurrentItem();
                    ViewGroup group = (ViewGroup) ((ViewGroup)mRefreshableView).getChildAt(position);
                    View scrollable = group.getChildAt(1);
                    if (!scrollable.canScrollVertically(-1)) {
                        mLastMotionY = event.getY();
