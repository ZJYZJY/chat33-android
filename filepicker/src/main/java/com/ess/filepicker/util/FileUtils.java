...
public final class FileUtils {
    ...
    public static final String INTERNAL_CARD = FzmFramework.getString(R.string.file_picker_internal_card);
    public static final String SD_CARD = FzmFramework.getString(R.string.file_picker_sd_card);
    ...
    public static List<BreadModel> getBreadModeListFromPath(List<String> mSdCardList, String path) {
        ...
        for (int i = 0; i < mSdCardList.size(); i++) {
            if (i == 0) {
                //内部存储设备
                path = path.replace(mSdCardList.get(i), "/" + INTERNAL_CARD);
            } else {
                path = path.replace(mSdCardList.get(i), "/" + SD_CARD + i);
            }
        }
    ...
    public static String getBreadModelListByPosition(List<String> mSdCardList, List<BreadModel> breadModelList, int position) {
        ...
        if (resultStr.startsWith("/" + INTERNAL_CARD)) {
            resultStr = resultStr.replace("/" + INTERNAL_CARD, mSdCardList.get(0));
        } else if (resultStr.startsWith("/" + SD_CARD)) {
            resultStr = resultStr.replace("/" + SD_CARD + String.valueOf(resultStr.charAt(4)), mSdCardList.get(Integer.valueOf(String.valueOf(resultStr.charAt(4)))));
        }
        return resultStr;
    }
    ...
    public static String getChangeSdCard(String sdCardName,List<String> mSdCardList){
        ...
        if(sdCardName.startsWith(INTERNAL_CARD)){
            return mSdCardList.get(0) + File.separator;
        }
        if(sdCardName.startsWith(SD_CARD)){
    ...
    public static List<String> getAllSdCardList(List<String> sdCardList) {
        ...
        for (int i = 0; i < sdCardList.size(); i++) {
            if (i == 0) {
                //内部存储设备
                resultList.add(INTERNAL_CARD);
            } else {
                resultList.add(SD_CARD + i);
            }
        }
        return resultList;
    }
