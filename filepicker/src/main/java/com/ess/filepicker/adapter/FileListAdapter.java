...
public class FileListAdapter extends BaseQuickAdapter<EssFile, BaseViewHolder> {
    ...
    protected void convert(BaseViewHolder helper, EssFile item) {
        ...
        if (item.isDirectory()) {
            ...
            if(item.getChildFolderCount().equals(mContext.getString(R.string.file_picker_loading))){
                //查找数量
                if(loadFileCountListener!=null){
                    loadFileCountListener.onLoadFileCount(helper.getAdapterPosition());
                }
            }
            textView.setText(String.format(mContext.getString(R.string.file_picker_folder_desc), item.getChildFileCount(),item.getChildFolderCount()));
        } else {
            helper.setVisible(R.id.iv_item_file_select_right, false);
            textView.setText(String.format(mContext.getString(R.string.file_picker_file_desc), FileUtils.getDateTime(item.getAbsolutePath()), FileSizeUtil.getAutoFileOrFilesSize(item.getFile())));
        }
